...
$docdata = mysql_fetch_assoc($qq);
	...
	else
	{
	  ...
	  $iiid = mysql_insert_id();	
		...
		/* Delete existing to add new all */
...
for($i=0;$i<$c;$i++)
{
	...
	$download = $selectboxes[$i];
  	 ...
  	 mysql_query("insert into `".$itable."` set  `pid` = '".$iiid."', `title` = '".$title."', `artist` = '".$artist."', `song` = '".$song."', `image` = '".$image."', `download` = '".$download."', `adddate` = now() ");
